...
class modified_text_classification(DLStudio.TextClassification):
    def run_code_for_training_with_TEXTnetOrder2(self, net, hidden_size):        
            filename_for_out = "training_loss_batching_mean_length_textnet2" + str(self.dl_studio.epochs) + "epochs.txt"
            ...
            plt.legend()
            plt.savefig("training_loss_batching_mean_length_textnet2.png")
            plt.show()
...
class modified_sentiment_analysis(DLStudio.TextClassification.SentimentAnalysisDataset):
    ...
    def review_to_tensor(self, review):
        review_tensor = torch.zeros(self.get_mean_review_length(), len(self.vocab))
